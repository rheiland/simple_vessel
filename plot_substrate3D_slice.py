...
# dx,dy,dz=20
    ...
    Ny=10
    Nx=50
    # grid2D = M[0,:].reshape(Nx,Ny,Nz)
    grid2D = M[0,:].reshape(Nz,Ny,Nx)

    #  x:[-500,500], y:[-100,100], z:[-50,50]
    xvec = grid2D[0,0,:]
    print('xvec= ',xvec)
    # xvec=  [-490. -470. -450.  ... 490]
    print('len(xvec)= ',len(xvec))  # 50
    print('type(xvec)= ',type(xvec))  # 
    yvec = grid2D[0,:,0]
    # yvec = [-80 -60 -40 -20 -10 10 20 40 60 80]
    yvec = np.array([-80, -60, -40, -20, -10, 10, 20, 40, 60, 80])
    print('yvec= ',yvec)  # 10
    print('len(yvec)= ',len(yvec))
    #xvec.size
    #xvec.shape
    # num_contours = 30
    num_contours = 10
#    my_plot = plt.contourf(xvec,xvec,M[field_index,:].reshape(100,100), num_contours, cmap='viridis') #'viridis'
    ...
    # substrate = M[field_index,:].reshape(Nx,Ny,Nz)
    substrate = M[field_index,:].reshape(Nz,Ny,Nx)
    print("min,max of substrate slice= ",substrate.min(),substrate.max())
    # print('substrate[:,:,2]= ',substrate[:,:,2])
    # print('substrate[2,:,:]= ',substrate[2,:,:])
    ...
    my_plot = plt.contourf(xvec,yvec, substrate[2,:,:], num_contours, cmap='viridis') #'viridis'
#    cbar.remove()
    if cbar == None:
